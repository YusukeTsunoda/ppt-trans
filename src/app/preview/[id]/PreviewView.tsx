...
export default function PreviewView({ file }: PreviewViewProps) {
  ...
  const extractText = async () => {
    ...
    try {
      ...
      // スライドデータを整形
      const formattedSlides: SlideData[] = extractedDataResult.slides.map((slide: ExtractedData['slides'][0]) => {
        const texts: any[] = [];
        let textIndex = 0;
      ...
      await supabase
        .from('files')
        .update({
          extracted_data: extractedDataResult,
          status: 'processed',
        })
        .eq('id', file.id);
      ...
      const errorMessage = err?.message || err?.error || 'テキスト抽出中にエラーが発生しました';
